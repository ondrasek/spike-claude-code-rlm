...
- `--structured-output probe` (default) sends a cheap echo-JSON request before the main loop to test if the backend produces valid structured output. The result is cached on the RLM instance. `on` skips the probe; `off` disables structured output entirely (regex-only). This prevents regressions on Ollama and other backends that claim JSON support but produce invalid output.
