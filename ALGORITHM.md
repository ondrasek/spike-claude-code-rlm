...
- **Structured output backend implementations** — `OpenAICompatibleBackend` now implements `structured_completion()` with JSON mode; `AnthropicBackend` still needs its own implementation; orchestrator integration (#14) needed to actually use structured output in the iteration loop
