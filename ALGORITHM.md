...
| Aspect | Root LM (Context-Engineer) | Sub-LM (Worker) |
|--------|---------------------------|-----------------|
...
| Model | `--model` | `--recursive-model` (defaults to `--model`) |

### Root LM Responsibilities

The root LM is the "context-engineer" — it writes Python code to explore the document
...
3. Delegates **semantic** analysis to the sub-LM via `llm_query(snippet, task)`
4. Aggregates sub-LM results and calls `FINAL(answer)` when confident

### Sub-LM Responsibilities

The sub-LM is a "worker" that processes individual text chunks. It:
...
| **Summarization** | Hierarchical summarization via recursive sub-calls | Summarize chunks, then summarize the summaries |
